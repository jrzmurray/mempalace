...
def _apply_exclude_patterns_to_prescanned_files(
    files: list, project_path: Path, include_ignored: list, exclude_patterns: list
) -> list:
    """Filter a caller-provided (pre-scanned) file list by ``exclude_patterns``.

    ``scan_project`` applies ``exclude_patterns`` itself, but callers of
    ``_mine_impl`` that pass their own ``files`` list bypass that scan
    entirely. Re-applying the filter here keeps per-project exclusions in
    effect regardless of how the file list was produced. ``force_include``
    paths (via ``include_ignored``) are preserved either way.
    """
    include_paths = normalize_include_paths(include_ignored)
...
def _mine_impl(
    ...
    if files is None:
        ...
        files = _apply_exclude_patterns_to_prescanned_files(
            files, project_path, include_ignored, exclude_patterns
        )
